...
def WB(R,RX,RY):     #Wait_Button - ожидание кнопки. R-тон красного (RGB)/ RX,RY - координаты, где искать
    a=(0,0,0)
    key=0
    while (a[0])!=R and key<100:
        im = pyautogui.screenshot()
        a = im.getpixel((RX, RY))
        key+=1               #засчита от зацикливания
...
for k in range(148):                       #кол-во циклов поиска
    print ("error=",error)
    ...
    for ii in range(1,131):#  ii=1         #основные мобы. 15 зеленый, переделать 51,37 - краснчерн. переделать
...
#file.close()                                 #Закрытие фаила лога
time.sleep(3)
