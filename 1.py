...
def search(ii,k): #поиск моба в нейтрале
    print('search',ii)
    if k==0 : #каджый 5 цикл проверяем на редких мобов с отдельным видом боя
    ...
    while mobl:
        ...
        if pyautogui.locateOnScreen('mana60.png', confidence=0.9,region=(969,1183, 110, 55)):
            ...
            if mobl:
                x,y=pyautogui.center(mobl)
                click (x,y,0.2)
                time.sleep(0.7)# потому что иногда анимация большая, тогда добавляет еще одну банку
        time.sleep(0.7)
        #атака
...
for k in range(200):
    print ("error=",error)
    if error==0:   #проверка на выпавшие не предусмотренные меню.
        proverka()
    error=0
    for ii in range(1,7):#  ii=1 #МАГИЯ(редкие мобы) номер моба, который в поиске 15 зеленый, переделать 51 - краснчерн. переделать
        error=error+search(ii,0)
    for ii in range(1,126):#  ii=1 #номер моба, который в поиске 15 зеленый, переделать 51,37 - краснчерн. переделать
